...
def check_board (board):
    ...
    if x_won and o_won:
        ...
        result = 'Player x has won, Congratulations!'
    elif o_won:
        result = 'Player o has won, Congratulations!'
    else:
        result = 'No player has won'
    return result

def check_board_bool (board, player): #checks if player has won
    x_won = False
    o_won = False
    if check_diag_1('x', board):
        x_won = True
        ...
        if not x_won and not o_won:
            for i in range(len(board)):
                if check_column('x', board, i):
                    ...
                    o_won = True
    if (player == 'x' and x_won) or (player == 'o' and o_won):
        result = True
    else:
        result = False
    return result
